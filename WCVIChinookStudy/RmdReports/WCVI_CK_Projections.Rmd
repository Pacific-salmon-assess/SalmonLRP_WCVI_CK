...
## Approach
Model parameters were adjusted so that resulting correlations among inlets in simulated spawner abundancesmatched the observed correlations in spawner abundances among inlets (Fig. 2).
...
When we compared the observed correlations in spawner abundances to simulated spawner correlations, we found that assuming correlations in recruitment residuals equal to estimated spawner correlations (scalar = 1) resulted in simulated spawner correlations that best matched observed correlations (Fig. 7). Provisional LRPs at p = 0.5 (50% probability that all inlets are above lower benchmarks) vary by approximately +/- 1000 fish and tend to be higher with lower correlations among inlets (lower scalar).
...
```{r echo=FALSE, fig.cap="**Figure 9**. Distribution of correlations of spawner abundances among inlets for observed data (n=10 pairwise correlations) and simulated time-series, for the assumption of variable age proportions among inlets and constant proportions among inlest. Projections assume a cv in exploitation rates among inlets = 0.21 (half that of estimated interannual variablity in exploitation rates). Provisional LRPs at p = 0.5 (50% probability that all inlets are above lower benchamrks) range between 10,300 (scalar = 1) and 11,700 (scalar = 0).", out.width = '60%'}
...
```{r echo=FALSE, fig.cap="**Figure 11**. Probability of all inlets being above their lower benchmark vs aggregate abundances from projections over 100 years and 2000 MC Trials, assuming variability in ER's among inlets with cv=0. LRPs at 4 probability values are highlighted, p=0.5 (yellow), p=0.66 (green), p=0.9 (light blue), and p=0.99(light purple). LRPs at p = 0.66, 0.9 and 0.99 require more MC trials for stabilization.", out.width = '60%'}

knitr::include_graphics(paste(wcviCKDir,"/Figures/ProjectedLRPs/cvER0n5000_ProjLRPCurve_ALLp.png",sep=""))

```
```{r echo=FALSE, fig.cap="**Figure 12**. Probability of all inlets being above their lower benchmark vs aggregate abundances from projections over 100 years and 2000 MC Trials, assuming variability in ER's among inlets with cv=0.42. LRPs at 4 probability values are highlighted, p=0.5 (yellow), p=0.66 (green), p=0.9 (light blue), and p=0.99(light purple). LRPs at p = 0.66, 0.9 and 0.99 require more MC trials for stabilization.", out.width = '60%'}
# Figure of logistic curve all p values for base case
knitr::include_graphics(paste(wcviCKDir,"/Figures/ProjectedLRPs/cvER0.42n5000_ProjLRPCurve_ALLp.png",sep=""))
```
...
```{r echo=FALSE, fig.cap="**Figure 13**. Time-series of aggregate escapement for WCVI Chinook (indicator stocks with PNI > 0.5), with LRPs at various probabilities of component inlets being > lower benchmarks: p=0.5 (yellow), p=0.66 (green), p=0.9 (light blue), and p=0.99 (light purple). LRPs at p = 0.66, 0.9 and 0.99 required more MC trials for stabilization."}
