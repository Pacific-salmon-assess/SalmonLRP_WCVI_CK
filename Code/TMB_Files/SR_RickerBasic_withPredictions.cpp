...
Type objective_function<Type>:: operator() ()
{
  ...
  for (int j = 0; j<N_predS; j++){
    if(biasCor == 0) LogR_Pred2(j) = logA(stk_pred(j)) + log(predS(j)) - exp(logB(stk_pred(j))) * predS(j);
    if(biasCor == 1) LogR_Pred2(j) = logA(stk_pred(j)) + log(predS(j)) - exp(logB(stk_pred(j))) * predS(j) - pow(sigma(stk_pred(j)),2)/Type(2);
  }
