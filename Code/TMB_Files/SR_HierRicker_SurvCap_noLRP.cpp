...
Type objective_function<Type>::operator() ()
{
  ...
  for(int i=0; i<N_Obs; i++){
    ...
    if (BiasCorrect == 1) {
      LogR_Pred_3(i) = logA(stk(i)) + gamma*logSurv_3(i) + log(P_3(i)*S(i)) - B(stk(i)) * S(i)- pow(sigma(stk(i)),2)/2;
      LogR_Pred_4(i) = logA(stk(i)) + gamma*logSurv_4(i) + log((1-P_3(i))*S(i)    ) - B(stk(i)) * S(i)- pow(sigma(stk(i)),2)/2;
    }
    if (BiasCorrect == 0) {
