...
run_ScenarioProj <- function(SRDat, EscDat, BMmodel, scenarioName, useGenMean, genYrs, TMB_Inputs, outDir,
  ...
  if (runMCMC == T) {
    ...
    #mcmcOut<-get_MCMC_Fit2(SRDat, BMmodel, TMB_Inputs, outDir,scenarioName, nMCMC=nMCMC)
...
# 
# out1<-post_long %>% left_join(muLSurv)
# 
...
get_MCMC_Fit2<-function (SRDat, BMmodel, TMB_Inputs, outDir,
                         scenarioName,nMCMC) {
  ...
  # # Fit mcmc with STAN to get parameter estimates for projections ===============================
  fitmcmc <- tmbstan(obj, chains=3, iter=nMCMC, init=obj$par,
