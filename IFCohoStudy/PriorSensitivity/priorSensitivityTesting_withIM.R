...
# --> Comparison of hierarchical and individual model approaches for SR model component
...
# Compile and load LRP estimation models written in TMB#dyn.unload(dynlib("TMB_Files/Aggregate_LRPs_Hier_Surv"))
...
# Call base function to fit hierarchical LRP model, where "Run_Ricker_LRP" comes from "LRPFunctions.r"
...
# Fit individual models for three scenarios based on value of Tau_dist  ===================================
...
# Run sensitivity analysis on hierarchical model prior choice =============================================
  ...
  for(tau_p in 1:length(IG_params)){
    for(gs in 1:length(gamma_sigs)){
      ...
      TMB_Inputs <- list(Scale = 1000, logA_Start = 1, logMuA_mean = logMuA_mean[lmm], logMuA_sig = logMuA_sig[lms], 
                         ...
                         gamma_mean = 0, gamma_sig = gamma_sigs[gs], S_dep = 1000, Sgen_sig = 0.5)
    
      # Call function to fit hierarchical LRP model for sensitivity scenario:
...
plot_LRPs(LRPs_tau, SDs_tau, legend_text_tau, name = "Comp_tau_withIM")
...
# # Extract individual model estimates and add to Ests_Tau
 ...
 Ests_Tau<-rbind(Ests_Tau,IM.rows)


PlotEsts(Ests = Ests_Tau, param = "alpha", legend_text_tau, name = "Comp_Tau_withIM")
PlotEsts(Ests = Ests_Tau, param = "Sgen", legend_text_tau, name = "Comp_Tau_withIM")
PlotEsts(Ests = Ests_Tau, param = "beta", legend_text_tau, name = "Comp_Tau_withIM")
PlotEsts(Ests = Ests_Tau, param = "sigma", legend_text_tau, name = "Comp_Tau_withIM")
