...
# check_0_escape <- ldat [ intersect( which(ldat$NME %in% ldat$NME[which(ldat$Escape ==0)]), which(ldat$Year %in% ldat$Year[which(ldat$Escape==0)]) ), ]
# write.csv(check_0_escape, "DataOut/check_0_escape.csv", row.names=FALSE )
...
# # check infilling
# temp <- AllDat[[2]]
# ggplot(temp, aes(y=GroupEsc, x=as.integer(Year), group=CU_Name)) +
